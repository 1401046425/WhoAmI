...
public class ScratchCard : MonoBehaviour
{
	...
	{   
		...
		if (!Input.touchSupported || Input.mousePresent)
		{
			...
			{
				OnScratch(new Vector3(Input.mousePosition.x,Input.mousePosition.y,selfScenePosition.z));
			}
		}
		if (Input.touchSupported)
		{
			foreach (Touch touch in Input.touches)
			{
				...
				if (touch.fingerId == fingerId)
				{
					...
					{
						OnScratch(new  Vector3(touch.position.x,touch.position.y,selfScenePosition.z));
					}
	...
	private void OnScratch(Vector3 position)
	{
